...
def reaudio(source: Path, dest: Path, args: argparse.Namespace) -> None:
    fh, _tmpfile = tempfile.mkstemp(suffix=dest.suffix, prefix="reaudio_", dir=dest.parent)
    os.close(fh)
    ...
    cmd = [
        ...
        "-i", source,  # input
        "-c:v", "copy",  # video
        "-c:a", "aac", "-b:a", "320k",  # audio
        "-y", tmpfile,  # output
    ]

    try:
        subprocess.run(cmd, shell=False, stdin=subprocess.DEVNULL,
                       check=True, timeout=args.timeout)
    except FileNotFoundError:
        print("ERROR: couldn't execute ffmpeg, please make sure it exists in your PATH")
        sys.exit(1)
    except subprocess.TimeoutExpired:
        print(f"ERROR: reaudio process timed out after {args.timeout} seconds", file=sys.stderr)
        tmpfile.unlink(missing_ok=True)
        sys.exit(1)
    except subprocess.CalledProcessError as e:
        print(
            f"ERROR: reaudio process failed with ffmpeg exit code {e.returncode}", file=sys.stderr)
        tmpfile.unlink(missing_ok=True)
        sys.exit(1)
    except Exception:
        print("ERROR: unknown error during reaudio process, aborting", file=sys.stderr)
        tmpfile.unlink(missing_ok=True)
        sys.exit(1)
    ...
    print(f"\nDONE: completed reaudio to {dest}")
...
def parse_arguments(argv: List[str]):
    parser = argparse.ArgumentParser(
        prog='reaudio.py',
        description='Remux a video file with audio re-encoded to AAC',
    )
    ...
    parser.add_argument(
        ...
        action=NegateAction,
        nargs=0,
        help="delete original file after regenerating audio"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10 * 60,
        help="timeout for reaudio operation",
    )
...
def main(argv: List[str]) -> int:
    ...
    reaudio(args.file, args.out, args)
